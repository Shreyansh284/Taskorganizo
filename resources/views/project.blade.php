...
<div class="todo">
    ...
    <ul class="todo-list">
        ...
        <li class="high">
            <a href="{{ URL::to('/') }}/project/{{$project->project_name}}" style="text-decoration: none; color:rgb(76, 76, 76) " >  <strong>{{$project->project_name}}</strong>    </a>
...
<div class="modal fade" id="addproject" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="false">
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form method="POST"  action="{{ URL::to('/') }}/editProject" class="register-form" id="login-form">
              @csrf
            <div class="form-group">
              <input type="text" class="form-control" id="recipient-name" placeholder="Project Name" name="project_name"  value="{{ old('project_name') }}" required>
              <span>
                  @error('project_name')
                 {{$message}}
                  @enderror
